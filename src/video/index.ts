...
export async function checkAnchorVideos(
  ...
  try {
    ...
    // ---------- 首次运行：只处理最新 1 条 ----------
    if (!lastTime) {
      if (allVideos.length === 0) { logger.info(anchor.name, '[VIDEO] 没有视频'); return; }
      const latest = allVideos[0];
      logger.info(anchor.name, `[VIDEO] 首次运行，只处理最新 1 条 (${latest.dirTag})`);
      const ok = await processOne(anchor, latest, onProcessed, cookie);
      if (ok) {
        setLastTime(anchor.name, latest.dirTag);
        logger.info(anchor.name, `[VIDEO] 首次处理完成`);
      } else {
        logger.warn(anchor.name, `[VIDEO] 首次处理失败，基准时间不推进，下次重试`);
      }
      return;
    }
    ...
    if (toProcess.length === 0) { logger.info(anchor.name, '[VIDEO] 没有新视频'); return; }

    // 从旧到新处理
    toProcess.sort((a, b) => a.createTime - b.createTime);
    logger.info(anchor.name, `[VIDEO] 待处理: ${toProcess.map(v => v.dirTag).join(', ')}`);

    let lastSuccessTag = lastTime;
    for (const target of toProcess) {
      const ok = await processOne(anchor, target, onProcessed, cookie);
      if (ok) {
        lastSuccessTag = target.dirTag;
        // 每条成功就推进一次，防止中途崩溃全部重来
        setLastTime(anchor.name, lastSuccessTag);
      } else {
        logger.warn(anchor.name, `[VIDEO] ${target.dirTag} 处理失败，停止推进，下次从这里重试`);
        break;
      }
    }

    logger.info(anchor.name, `[VIDEO] ===== 检查完成 (基准=${lastSuccessTag}) =====`);
  } catch (e: any) {
...
/**
 * 处理单个视频
 * @returns true=成功（可推进基准），false=失败（不推进，下次重试）
 */
async function processOne(
  ...
  try {
    ...
    const pa = aweme?.video?.play_addr?.url_list;
    if (Array.isArray(pa)) candidates.push(...pa);
    for (const br of aweme?.video?.bit_rate ?? []) {
      const ul = br?.play_addr?.url_list;
      if (Array.isArray(ul)) candidates.push(...ul);
    }
    const playUrls = [...new Set(candidates)].filter(Boolean);
    if (playUrls.length === 0) {
      logger.error(anchor.name, `[VIDEO] 无播放地址 ${videoId}`);
      return false;
    }
    logger.info(anchor.name, `[VIDEO] 候选播放地址 ${playUrls.length} 个`);
    ...
    for (let i = 0; i < playUrls.length; i++) {
      try {
        logger.info(anchor.name, `[VIDEO] 尝试地址 ${i + 1}/${playUrls.length}: ${playUrls[i].slice(0, 80)}...`);
        await downloadToFile(playUrls[i], videoPath, cookie);
        downloaded = true;
        break;
      } catch (e: any) {
        lastDlErr = e;
        logger.warn(anchor.name, `[VIDEO] 地址 ${i + 1} 失败: ${e.message}`);
      }
    }
    if (!downloaded) {
      throw new Error(`所有 ${playUrls.length} 个地址都下载失败: ${lastDlErr?.message || 'unknown'}`);
    }
    ...
    if (VIDEO_ASR_MODE === 'stream') {
      ...
      try {
        ...
        logger.warn(anchor.name, `[VIDEO] Gemini Live 失败，降级 Groq/SenseVoice: ${e.message}`);
        const { transcribe } = await import('../asr/sensevoice.js');
        text = await transcribe(audioPath);
        logger.info(anchor.name, `[VIDEO] ASR (Groq/SenseVoice) 完成 (${((Date.now() - t0) / 1000).toFixed(1)}s, ${text.length} 字)`);
      }
    } else {
      logger.info(anchor.name, `[VIDEO] ASR (Groq/SenseVoice) ...`);
      try {
        const { transcribe } = await import('../asr/sensevoice.js');
        text = await transcribe(audioPath);
        logger.info(anchor.name, `[VIDEO] ASR (Groq/SenseVoice) 完成 (${((Date.now() - t0) / 1000).toFixed(1)}s, ${text.length} 字)`);
      } catch (e: any) {
        logger.warn(anchor.name, `[VIDEO] Groq/SenseVoice 失败，降级 Gemini Live: ${e.message}`);
        const { transcribeFile } = await import('../asr/gemini-live-file.js');
        text = await transcribeFile(audioPath);
        logger.info(anchor.name, `[VIDEO] ASR (Gemini Live) 完成 (${((Date.now() - t0) / 1000).toFixed(1)}s, ${text.length} 字)`);
      }
...
/**
 * 下载文件（带 Cookie + Referer，解决 403）
 */
async function downloadToFile(url: string, dest: string, cookie: string): Promise<void> {
  const resp = await fetch(url, {
    headers: {
      'User-Agent': UA,
      'Referer': 'https://www.douyin.com/',
  ...
  if (!resp.ok) {
    ...
    throw new Error(`下载失败 HTTP ${resp.status} ${body.slice(0, 200)}`);
  }
