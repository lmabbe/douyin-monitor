...
async function processOne(
  ...
  try {
    ...
    if (!playAddr) { logger.error(anchor.name, `[VIDEO] 无播放地址 ${videoId}`); return; }
    ...
    const tAI = Date.now();
    try {
      logger.info(anchor.name, `[VIDEO] AI 总结 (Gemini) ...`);
      const { summarizeSegment } = await import('../ai/gemini.js');
      summary = await summarizeSegment(text);
      logger.info(anchor.name, `[VIDEO] AI 总结 (Gemini) 完成 (${((Date.now()-tAI)/1000).toFixed(1)}s, ${summary.length} 字)`);
    } catch (e: any) {
      logger.warn(anchor.name, `[VIDEO] Gemini 失败: ${e.message}`);
      try {
        logger.info(anchor.name, `[VIDEO] 降级到腾讯 ...`);
        const { summarizeSegment } = await import('../ai/tencent.js');
        summary = await summarizeSegment(text);
        logger.info(anchor.name, `[VIDEO] AI 总结 (腾讯) 完成 (${((Date.now()-tAI)/1000).toFixed(1)}s, ${summary.length} 字)`);
      } catch (e2: any) {
